...
function accuracy(w_gen, w_disc, xtst, epoch)
    ...
    #image = xtst[i];
    #open("test2.raw", "w");
    #image = round(Int, image.*255);
    #image = convert(Array{UInt8}, image);
    #write("test2.raw", image[:,:,1,1])
    ncorrect1 += sum(ypred[1:1,:] .> 0.5)
    ninstance += size(ypred,2)
    noise = randn(Float32,100,128)
    fake_image2 = generator(w_gen, noise;mode=1)
    fake_image = generator(w_gen, noise;mode=0)
    #open("test_epoch$(epoch)_$(i).raw", "a+")
    #png = round(Int, fake_image2[:,:,:,1].*255);
    #png = makegrid(fake_image2; gridsize=[8,8], scale=2.0)
    #fake_image2 = convert(Array{Float64}, fake_image2)
    #fake_image2 = round(Int, fake_image2[:,:,:,1].*255);
    #fake_image2 = convert(Array{UInt8}, fake_image2);
    #png = convert(Image, fake_image2)
    #png = colorim(fake_image2)
    ...
    #fake_image1 = convert(Array{UInt8}, fake_image1);
    #write("test_epoch$(epoch)_$(i).raw", fake_image1[:,:,1,1])
